...
function touchEnded() {
 ...
 window.addEventListener("keydown", function(e) {
  ...
  var jumpgeo = new THREE.ConeBufferGeometry( 0.5, 1, 6 );
  var jumpmat = new THREE.MeshLambertMaterial( { color: "rgb(55,255,55)" } );
  var jump = new THREE.Mesh( jumpgeo, jumpmat );
  jump.position.y = 150;
  jump.position.x = ( Math.random() - 0.5 ) * 100;
  jump.position.z = 0.25;
  jump.rotation.x = Math.radians(90);
  scene.add( jump );

  var geom = new THREE.BoxBufferGeometry( 1, 1, 1 );
  var mate = new THREE.MeshLambertMaterial( {color: "rgb(25,25,255)", reflectivity: 1.0 } );
  var blocks = [];
  for ( var i = 0; i < 100; i++ ) {
  ...
  var planemat = new THREE.MeshLambertMaterial( { color: "rgb(255,255,255)" } )
  var plane = new THREE.Mesh( planegeo, planemat );
  plane.position.set( 0, 0, -0.5 );
  scene.add( plane );

  var amblight = new THREE.AmbientLight( "rgb(10,10,10)", 2 );
  scene.add( amblight );
  ...
  var thrustlight = new THREE.PointLight( "rgb(55,255,55)", 3, 10, 2 );
    ...
    if ( pause == false && lives > 0 ) {
        for (var i = 0; i < blocks.length; i++ ) {
          ...
          if (blocks[i].position.y<-15) {
            ...
            blocks[i].position.x = ( Math.random() - 0.5) * 100;
          }
          if ( dist(light.position.x, light.position.y, light.position.z, blocks[i].position.x, blocks[i].position.y, blocks[i].position.z) < 1 && lvlSpeed > 0) {
        ...
        for ( var i = 0; i < thrust.length; i++ ) {
          ...
          thrust[i].material.opacity = thrustlight.intensity/10;
        ...
        jump.position.y -= lvlSpeed;
        jump.rotation.y += Math.radians(3);
        if ( jump.position.y < -1250 ) {
          jump.position.y = 50;
          jump.position.x = ( Math.random() - 0.5 ) * 25;
        }
        if ( dist( jump.position.x, jump.position.y, jump.position.z, light.position.x, light.position.y, light.position.z ) < 1.75) {
          jumpVel = 0.25;
          lvlSpeed += 0.2;
          console.log("jump");
        }
        ...
        if ( keyboard.pressed("right") || keyboard.pressed("D") || playerX > window.innerWidth/2 ) {
          playerVel += 0.06;
        }
        if ( keyboard.pressed("left")  || keyboard.pressed("A") || playerX < window.innerWidth/2 ) {
          playerVel -= 0.06;
        }
        ...
        camera.rotation.y = (lpoint.position.z+2)/20;
        camera.position.y = (lpoint.position.z+2)/20-3;
